...
enum lpf_e {
    INV_FILTER_256HZ_NOLPF2 = 0,		//参考enum类型，下面的变量会自动递增1
    INV_FILTER_188HZ,
    INV_FILTER_98HZ,
    INV_FILTER_42HZ,
    INV_FILTER_20HZ,
...
static int set_int_enable(unsigned char enable)
{
    ...
    if (st.chip_cfg.dmp_on) {
        ...
        if (enable)
            ...
            tmp = 0x00;
        if (i2c_write(st.hw->addr, st.reg->int_enable, 1, &tmp))	//开启Sensor Register Raw Data sensors are updated 															
            return -1;												//and Ready to be read中断	
        st.chip_cfg.int_enable = tmp;		//记录中断状态
    }
    return 0;
}
...
//	i2c_read(st.hw->addr, st.reg->who_am_i, 1, &id);
//	printf("ID:%d",id);
...
//    if (i2c_write(st.hw->addr, st.reg->accel_cfg2, 1, data))	//这个寄存器和书上写的也不一样啊...
//        return -1;
#endif
    ...
    if (mpu_set_sample_rate(50))
        return -1;
    if (mpu_configure_fifo(0))			//此处并没有配置任何寄存器啊...原来是零的现在还是零..
        return -1;
...
int mpu_reset_fifo(void)
{
    ...
    data = 0;
    if (i2c_write(st.hw->addr, st.reg->int_enable, 1, &data))	//先把所有功能关闭
        return -1;
    ...
    if (st.chip_cfg.dmp_on) {
        ...
        if (i2c_write(st.hw->addr, st.reg->user_ctrl, 1, &data))	//reset fifo
            return -1;
        if (st.chip_cfg.bypass_mode || !(st.chip_cfg.sensors & INV_XYZ_COMPASS))	//open fifo
            data = BIT_FIFO_EN;
        else
        ...
        if (i2c_write(st.hw->addr, st.reg->int_enable, 1, &data))
            return -1;
        if (i2c_write(st.hw->addr, st.reg->fifo_en, 1, &st.chip_cfg.fifo_enable))	//fifo全部打开
            return -1;
    }
    return 0;
}
...
{
    switch (st.chip_cfg.gyro_fsr) {			//这是什么意思？好像是在哪里读取了fsr值？
    case INV_FSR_250DPS:
        fsr[0] = 250;
        break;
    case INV_FSR_500DPS:
...
int mpu_set_sample_rate(unsigned short rate)
{
    ...
    if (st.chip_cfg.dmp_on)	//在设置采样频率的时候dmp必须关闭
        return -1;
    else {
...
int mpu_set_compass_sample_rate(unsigned short rate)
{
    ...
    if (i2c_write(st.hw->addr, st.reg->s4_ctrl, 1, &div))		//此处感觉也有问题，为什么要把频率值放到Ctrl寄存器中呢？
        return -1;
...
int mpu_configure_fifo(unsigned char sensors)
{
    ...
    if (st.chip_cfg.dmp_on)		//要先配置FIFO才能打开DMP
        return 0;
    else {
        if (!(st.chip_cfg.sensors))		//在已经打开sensor的情况下才能配置fifo
            return -1;
        prev = st.chip_cfg.fifo_enable;		//读取配置前状态
        st.chip_cfg.fifo_enable = sensors & st.chip_cfg.sensors;	//记录要进入fifo的sensor，这里是gyro与accel所有
        if (st.chip_cfg.fifo_enable != sensors)
            /* You're not getting what you asked for. Some sensors are
             * asleep.
             */				//这里就不明白了，sensor与st.chip_cfg.fifo_enable都是手动输入的怎么可能不一样
            result = -1;
        else
            result = 0;
        if (sensors || st.chip_cfg.lp_accel_mode)
            set_int_enable(1);		//开启数据准备好了可以读取的中断
        else
            set_int_enable(0);
        if (sensors) {
            if (mpu_reset_fifo()) {
                st.chip_cfg.fifo_enable = prev;		//如果配置失败再记录原始状态即没有sensor进入fifo
                return -1;
            }
        }
    }
...
int mpu_set_sensors(unsigned char sensors)
{
    ...
    if (sensors & INV_XYZ_GYRO)
        ...
        data = BIT_SLEEP;				//初始化让mpu进入sleep mode
    if (i2c_write(st.hw->addr, st.reg->pwr_mgmt_1, 1, &data)) {		//配置时钟为pll自动模式
    ...
    if (i2c_write(st.hw->addr, st.reg->pwr_mgmt_2, 1, &data)) {
        st.chip_cfg.sensors = 0;		//data=0，所有sensor全部打开
        return -1;
    }
...
#ifdef AK89xx_SECONDARY		//这里大概是选择对compass的操作模式，可以是单片机直接和compass通信即bypass模式
#ifdef AK89xx_BYPASS		//也可以是mpu作为master、compass作为secondary的间接通信
    ...
    if (st.chip_cfg.dmp_on)
        ...
        user_ctrl &= ~BIT_DMP_EN;
    if (i2c_write(st.hw->addr, st.reg->s1_do, 1, &data))	//初始化使meg进入power down模式，然后如果sensor=compass可以直接进入single measurement
        return -1;
    /* Enable/disable I2C master mode. */
    if (i2c_write(st.hw->addr, st.reg->user_ctrl, 1, &user_ctrl))	//在不使用罗盘时关闭iic master，meg由单片机通信
        return -1;										//但是这里看不懂对于DMP_EN的操作，寄存器里根本没有那一位
#endif
#endif

    st.chip_cfg.sensors = sensors;	//记录此时sensor的开启状态
    st.chip_cfg.lp_accel_mode = 0;
    delay_ms(50);
    return 0;
}
...
int mpu_set_bypass(unsigned char bypass_on)
{
    ...
    if (bypass_on) {
        if (i2c_read(st.hw->addr, st.reg->user_ctrl, 1, &tmp))
            return -1;
        tmp &= ~BIT_AUX_IF_EN;	//确保disable the i2c master module，然后激活bypass这样就可以用iic直接访问compass
...
int mpu_run_self_test(long *gyro, long *accel)
{
    ...
    if (st.chip_cfg.dmp_on) {			//这一句还不太明白，并没有通过iic读取寄存器的操作那么读到的不还是原始值吗？
        mpu_set_dmp_state(0);
        dmp_was_on = 1;
    } else
        dmp_was_on = 0;
    ...
    mpu_get_gyro_fsr(&gyro_fsr);		//gyro的full scale range也即量程
    mpu_get_accel_fsr(&accel_fsr);		//accel的full scale range
    mpu_get_lpf(&lpf);
...
static int setup_compass(void)
{
    ...
    data[0] = AKM_POWER_DOWN;
    if (i2c_write(st.chip_cfg.compass_addr, AKM_REG_CNTL, 1, data))		//看起来是先通过bypass模式直接和磁力仪联通
        return -1;												//然后读取到补偿值之后就关闭bypass还是由mpu控制磁力仪
    delay_ms(1);
    ...
    data[0] = 0x40;
    if (i2c_write(st.hw->addr, st.reg->i2c_mst, 1, data))	//此时重新回到mpu的iic主模式，由mpu与磁力仪通信
        return -1;

    /* Slave 0 reads from AKM data registers. */
    data[0] = BIT_I2C_READ | st.chip_cfg.compass_addr;
    if (i2c_write(st.hw->addr, st.reg->s0_addr, 1, data))	//此处是将slave 0也就是磁力仪的地址配置到mpu的s0_addr
        return -1;										//但是不知道为什么原代码并没有初始化s0_addr，难道不用这个？

    /* Compass reads start at this register. */
    data[0] = AKM_REG_ST1;
    if (i2c_write(st.hw->addr, st.reg->s0_reg, 1, data))	//先读取mag的status1寄存器
        return -1;

    /* Enable slave 0, 8-byte reads. */
    data[0] = BIT_SLAVE_EN | 8;
    if (i2c_write(st.hw->addr, st.reg->s0_ctrl, 1, data))	//使能，定义读取数据为8位
        return -1;

    /* Slave 1 changes AKM measurement mode. */
    data[0] = st.chip_cfg.compass_addr;						//slave 1是mag的CNTL寄存器，要通过mpu写这个寄存器
    ...
    /* Set slave 1 data. */
    data[0] = AKM_SINGLE_MEASUREMENT;			//配置s1_data out寄存器把要写入mag的寄存器的数据写到这里面
    if (i2c_write(st.hw->addr, st.reg->s1_do, 1, data))		//不太明白single measurement什么意思
        return -1;

    /* Trigger slave 0 and slave 1 actions at each sample. */
    data[0] = 0x03;
    if (i2c_write(st.hw->addr, st.reg->i2c_delay_ctrl, 1, data))	//也不明白这个是干嘛的：When enabled, slave 0 will 
		 return -1;				//only be accessed 1+I2C_MST_DLY) samples as determined by SMPLRT_DIV and DLPF_CFG
